...
const QualitySettingsPage = () => {
  ...
  const updateName = async () => {
    ...
    if (error) { toast.error("حدث خطأ"); } else {
  ...
  return (
    <div>
      ...
      <div className="space-y-6 max-w-lg">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg"><User className="w-5 h-5" />تغيير الاسم</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div><Label>الاسم الكامل</Label><Input value={fullName} onChange={(e) => setFullName(e.target.value)} maxLength={100} /></div>
            <Button onClick={updateName} disabled={savingName}>{savingName ? "جارٍ الحفظ..." : "حفظ الاسم"}</Button>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg"><Lock className="w-5 h-5" />تغيير كلمة المرور</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <Button onClick={updatePassword} disabled={savingPassword}>{savingPassword ? "جارٍ الحفظ..." : "تحديث كلمة المرور"}</Button>
          </CardContent>
        </Card>
      </div>
    </div>
